...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  const bloomSound = useRef(new Audio('/boom.mp3'));
  const [webglError, setWebglError] = useState('');

  useEffect(() => {
    bloomSound.current.volume = 1.0; 
    
    return () => {
      if (bloomSound.current) {
        bloomSound.current.pause();
      }
    };
  }, []);
  ...
  useEffect(() => {
   ...
   class App {
      ...
      // CLASS METHODS YAHAN SE SHURU HOTE HAIN
      onWindowResize() {
      ...
      init() {
        ...
        this.initPasses();
        if (!this.renderPass || !this.bloomPass) return;
      ...
      onMouseDown(ev) {
        ...
        // Play sound on click
        bloomSound.current.currentTime = 0; 
        bloomSound.current.play().catch(() => {}); 
      }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Unable to create WebGL renderer.';
    ...
    myApp.loadAssets().then(() => {
      if (isCancelled || myApp.disposed) return;
      myApp.init();
    });
  ...
  // HyperSpeed.jsx ke bilkul niche:
  return (
    <div className="relative w-full h-full min-h-[600px]" ref={hyperspeed}>
      {webglError ? (
        <div className="absolute inset-0 flex items-center justify-center bg-black/95 px-6 text-center text-white">
          <div>
            <p className="text-xl font-semibold tracking-tight">HyperSpeed unavailable</p>
            <p className="mt-2 text-sm text-zinc-300">
              Browser ne new WebGL context block kar diya. Train/model scenes ya dusre canvases GPU limit hit kar rahe hain.
            </p>
          </div>
        </div>
      ) : null}
