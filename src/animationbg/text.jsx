...
const COLLISION_POINT = new THREE.Vector3(12, 0, 0); // logo position

// --- LOGO PARTICLE SYSTEM with ENHANCED HOVER (vortex + color) ---
// --- LOGO PARTICLE SYSTEM with FIXED HOVER (no wobble, correct tracking) ---
const LogoParticles = ({ progress }) => {
  ...
  // Track mouse in world space (plane at z=0)
  useEffect(() => {
  ...
  // Load image and create particles (unchanged)
  useEffect(() => {
    const img = new Image();
    img.src = "/ALC.png";
  ...
  // Enhanced shader with correct world-space distance and fading wave
    ...
    return new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Transform to world space for correct mouse distance
          vec4 worldPos = modelMatrix * vec4(morphedPos, 1.0);
          vec3 worldPosition = worldPos.xyz;
          
          // Only apply hover when logo is almost fully formed
          float hoverActive = smoothstep(0.98, 1.0, uProgress);
          
          // Vector from particle (world) to mouse (world)
          vec3 toMouse = mousePos - worldPosition;
          float dist = length(toMouse);
          float influence = smoothstep(hoverRadius, 0.0, dist) * hoverActive;
          
          // Direction and perpendicular for swirl
          vec3 dir = normalize(toMouse + 0.001);
          vec3 perp = vec3(-dir.y, dir.x, dir.z); // 2D swirl
          ...
          // Scale down near mouse
          float scaleFactor = 1.0 - influence * 0.6;
          
          // Apply displacement in world space, then transform back? Easier: apply to morphedPos directly
          // But morphedPos is local; we need to apply displacement in local space aligned with world? 
          // Simpler: apply displacement to worldPos then transform to local? Too complex.
          // We'll apply displacement to morphedPos, but direction dir is in world space, so we need to transform dir to local.
          // Instead, we can rotate dir by inverse model matrix, but that's heavy. Better to keep everything in world:
          // We'll compute final world position and then transform to NDC.
          
          // Let's build final world position with displacement, then convert to clip space.
          vec3 finalWorldPos = worldPosition + displacement;
          
          // Fade out wave effect as logo solidifies
          float waveStrength = max(0.0, 1.0 - uProgress * 1.2); // fades to 0 at uProgress~0.83
          float wave = sin(finalWorldPos.x * 2.0 + uTime * 2.0 + aRandom * 5.0);
          finalWorldPos.z += wave * 0.5 * waveStrength;
          ...
          // Color: base cyan, shift to gold near mouse
  ...
  useFrame(({ clock }) => {
    ...
    if (pointsRef.current) {
      ...
      pointsRef.current.material.uniforms.mousePos.value.copy(mouseWorldPos);
      ...
      // Stop rotation when fully formed (uProgress >= 1)
      if (reveal >= 1.0) {
        pointsRef.current.rotation.y = 0;
      } else {
        // Gentle rotation during formation only
...
// --- COMET COMPONENTS (unchanged except hiding after logo forms) ---
const HeroComet = ({ progress }) => {
  ...
  useFrame(() => {
    ...
    if (meshRef.current) {
      ...
      // Hide after logo starts forming
      if (p > 0.85) {
...
const MultiComets = ({ progress, centerRef }) => {
  ...
  const comets = useMemo(() => {
    const temp = [];
    // Spiral (just 1 for flair)
    for (let i = 0; i < 1; i++) {
      const dir = new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize();
      temp.push({ type: "spiral", direction: dir });
    }
    // 6 Main Comets that hit the Logo
    for (let i = 0; i < 6; i++) {
  ...
  useFrame(() => {
    ...
    cometRefs.current.forEach((mesh, i) => {
      ...
      // Hide all comets after logo starts forming
      if (p > 0.85) {
        mesh.visible = false;
        return;
      }
...
// --- CAMERA RIG — now stops at logo when fully revealed ---
const CameraRig = ({ progress, centerRef }) => {
  const { camera } = useThree();
  const targetX = COLLISION_POINT.x; // final logo X = 12

  useFrame(() => {
    const p = progress.current;
    if (p > 0.9) {
      // Lock camera onto logo
      camera.position.x += (targetX - camera.position.x) * 0.1;
    } else {
      // Follow the average comet position (if available)
